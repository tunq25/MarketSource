...
export function Overview({ stats, users, purchases, notifications }: OverviewProps) {
  return (
    <div className="space-y-6">
        ...
        <Card className="lg:col-span-2 neon-border-hover glass-panel text-slate-900 dark:text-slate-100">
          ...
          <CardContent>
            <div className="space-y-4 max-h-64 overflow-y-auto">
              {purchases
                // ✅ FIX: Fallback || 0 tránh crash khi purchaseDate null/undefined
                .sort((a, b) =>
                  new Date(b.purchaseDate || b.timestamp || b.created_at || 0).getTime() -
                  new Date(a.purchaseDate || a.timestamp || a.created_at || 0).getTime()
                )
                .slice(0, 10)
                .map((purchase) => (
                <div key={purchase.id} className="flex items-center space-x-4 p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-medium">{purchase.title || purchase.productTitle || 'Sản phẩm'}</p>
                    <p className="text-xs text-muted-foreground">
                      Khách hàng: {purchase.userName || purchase.userEmail || 'Unknown'}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {/* ✅ FIX: Guard date null */}
                      {purchase.purchaseDate || purchase.timestamp
                        ? new Date(purchase.purchaseDate || purchase.timestamp).toLocaleString('vi-VN')
                        : 'N/A'}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-medium text-green-600">
                      {/* ✅ FIX: Guard amount null */}
                      +{(purchase.amount || 0).toLocaleString('vi-VN')}đ
                    </p>
      ...
      {/* ✅ FIX: Card thông báo đặt NGOÀI grid (full width), không còn nằm lỗi trong grid 3 cột */}
      <Card className="neon-border-hover glass-panel text-slate-900 dark:text-slate-100">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            Thông báo gần đây
            <Badge variant="outline" className="ml-auto">
              {/* ✅ FIX: Optional chaining .type?.includes() tránh crash khi type null */}
