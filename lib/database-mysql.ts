...
function matchesToUniqueConstraint(match: string): string {
  ...
  const parts = match.match(/UNIQUE\s+KEY\s+\w+\s*\(([^)]+)\)/i);
  if (parts) {
    return `, UNIQUE (${parts[1]})`;
  }
  return '';
}
...
function convertToPostgresSql(sql: string): string {
  ...
  // Handle ENUM(...) -> VARCHAR(255) for PostgreSQL simplicity
  converted = converted.replace(/ENUM\s*\([^)]+\)/gi, 'VARCHAR(255)');
  ...
  converted = converted.replace(/ROW_FORMAT\s*=\s*\w+/gi, '');

  // Handle inline INDEX and UNIQUE KEY (basic support)
  // PostgreSQL does not support INDEX inside CREATE TABLE, must be separate CREATE INDEX
  // We'll remove them to avoid syntax errors, though the index won't be created here.
  converted = converted.replace(/,\s*INDEX\s+\w+\s*\([^)]+\)/gi, '');
  converted = converted.replace(/,\s*UNIQUE\s+KEY\s+\w+\s*\([^)]+\)/gi, (match) => {
    return matchesToUniqueConstraint(match);
  });

  // 4. Convert ? → $1, $2...
  let indexCounter = 1;
  converted = converted.replace(/\?/g, () => `$${indexCounter++}`);

  // 5. ✅ FIX: Auto-add RETURNING id cho INSERT nếu chưa có (để lấy insertId)
  ...
  return withTransaction(async (conn) => {
    ...
    for (const item of purchaseData.items) {
      ...
      const [prdRows]: any = await conn.query(
        "SELECT id, price, is_active, title FROM products WHERE id = ? AND deleted_at IS NULL FOR UPDATE",
        [productIdNum],
      )
      ...
      validatedItems.push({ id: productIdNum, amount: price * quantity })
    }
...
export async function getReferralByCodeMySQL(referralCode: string) {
  try {
    // Referral code can be the actual referral_code, uid, or email
    return await queryOne<any>(
      `SELECT id, email, username, name, referral_code, uid 
       FROM users 
       WHERE referral_code = ? OR uid = ? OR email = ?`,
